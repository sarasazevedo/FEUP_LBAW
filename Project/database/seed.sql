...
CREATE FUNCTION delete_group_notification_after_join() RETURNS TRIGGER AS $$
DECLARE
    notification_id INTEGER;
BEGIN
    -- Find the notification ID associated with the group join
    FOR notification_id IN
        SELECT n.id
        FROM notification n
        JOIN group_notification gn ON n.id = gn.id
...
CREATE TRIGGER after_delete_groupmember_join
BEFORE DELETE ON group_member
FOR EACH ROW
EXECUTE PROCEDURE delete_group_notification_after_join();
...
CREATE FUNCTION delete_group_notification_after_leave() RETURNS TRIGGER AS $$
DECLARE
    notification_id INTEGER;
BEGIN
    -- Find the notification ID associated with the group leave
    FOR notification_id IN
        SELECT n.id
        FROM notification n
        JOIN group_notification gn ON n.id = gn.id
...
CREATE TRIGGER after_delete_groupmember_leave
BEFORE DELETE ON group_member
FOR EACH ROW
EXECUTE PROCEDURE delete_group_notification_after_leave();
